 ...
 */
function fileSize(bytes, opt_decimals) {
 ...
 */
function isConvertableScaledNumber(val) {
 ...
 */
function stringToNumericValue(stringValue) {
  if (strings.endsWith(stringValue, 'B')) {
 ...
 * @return {number} Numeric value for string.
 */
function stringToNumBytes(stringValue) {
  return stringToNumericValue_(
      stringValue, NUMERIC_SCALES_BINARY_);
};
 ...
 */
function numericValueToString(val, opt_decimals) {
 ...
 * @private
 */
function numericValueToString_(
 ...
 * @private
 */
let SCALED_NUMERIC_RE_ = /^(-?\d+\.?\d*)([KMGTPEZYkmun]?)B?$/;

/**
 * Ordered list of scaling prefixes in decreasing order.
 * @private {Array<string>}
 */
let NUMERIC_SCALE_PREFIXES_ =
 ...
 * @private
 */
let FIRST_GRAPHEME_EXTEND_ = 0x300;
 ...
 * @private
 */
function insertWordBreaksGeneric_(
 ...
 */
function insertWordBreaks(str, opt_maxlen) {
 ...
 * @private
 */
function conservativelyHasGraphemeBreak_(
 ...
 * @deprecated Prefer wrapping with CSS word-wrap: break-word.
 */
function insertWordBreaksBasic(str, opt_maxlen) {
  return insertWordBreaksGeneric_(
 ...
 * @private
 */
let IS_IE8_OR_ABOVE_ = userAgent.IE;
 ...
 * @type {string}
 */
let WORD_BREAK_HTML = userAgent.WEBKIT ? '<wbr></wbr>' :
    IS_IE8_OR_ABOVE_                    ? '&#8203;' :
                                                      '<wbr>';
...
export {WORD_BREAK_HTML, fileSize, insertWordBreaks, insertWordBreaksBasic, isConvertableScaledNumber, numBytesToString, numericValueToString, stringToNumBytes, stringToNumericValue};