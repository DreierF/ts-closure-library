...
class SafeHtml {
  ...
  constructor(value, dir, token) {
    ...
    this.privateDoNotAccessOrElseSafeHtmlWrappedValue_ =
        (token === SafeHtml.CONSTRUCTOR_TOKEN_PRIVATE_) ? value : '';
 ...
 * @const
 */
SafeHtml.prototype.implementsGoogI18nBidiDirectionalString = true;
 ...
 * @const {boolean}
 */
SafeHtml.prototype.implementsGoogStringTypedString = true;
 ...
 * @return {!SafeHtml} The escaped text, wrapped as a SafeHtml.
 */
SafeHtml.htmlEscapePreservingNewlinesAndSpaces = function(
    textOrHtml) {
  if (textOrHtml instanceof SafeHtml) {
    return textOrHtml;
  }
 ...
 */
SafeHtml.createIframe = function(
 ...
 */
SafeHtml.createSandboxIframe = function(
    opt_src, opt_srcdoc, opt_attributes, opt_content) {
 ...
 * @return {!SafeHtml} The SafeHtml content with the tag.
 */
SafeHtml.createWithDir = function(
...
SafeHtml.concatWithDir = function(dir, var_args) {
  var html =
      SafeHtml.concat(Array.prototype.slice.call(arguments, 1));
  html.dir_ = dir;
  return html;
};
 ...
 * @const
 */
SafeHtml.CONSTRUCTOR_TOKEN_PRIVATE_ = {};
 ...
 * @package
 */
SafeHtml.createSafeHtmlSecurityPrivateDoNotAccessOrElse = function(
    html, dir) {
  const policy = trustedtypes.getPolicyPrivateDoNotAccessOrElse();
  const trustedHtml = policy ? policy.createHTML(html) : html;
  return new SafeHtml(
      trustedHtml, dir, SafeHtml.CONSTRUCTOR_TOKEN_PRIVATE_);
};
 ...
 * @package
 */
SafeHtml.createSafeHtmlTagSecurityPrivateDoNotAccessOrElse = function(
 ...
 * @package
 */
SafeHtml.combineAttributes = function(
    fixedAttributes, defaultAttributes, opt_attributes) {
...
SafeHtml.EMPTY = new SafeHtml(
    ...
    Dir.NEUTRAL, SafeHtml.CONSTRUCTOR_TOKEN_PRIVATE_);
