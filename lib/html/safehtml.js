...
import * as googObject from './../object/object.js';
...
class SafeHtml {
  ...
  static verifyTagName(tagName) {
    if (!VALID_NAMES_IN_TAG.test(tagName)) {
      throw new Error(
          ENABLE_ERROR_MESSAGES ? `Invalid tag name <${tagName}>.` :
                                           '');
    }
...
function getAttrNameAndValue(tagName, name, value) {
  ...
  if (value instanceof Const) {
    ...
    if (SUPPORT_STYLE_ATTRIBUTE) {
      ...
      throw new Error(
          ENABLE_ERROR_MESSAGES ? 'Attribute "style" not supported.' :
                                           '');
    }
...
export {SafeHtml, ENABLE_ERROR_MESSAGES, SUPPORT_STYLE_ATTRIBUTE};