...
class SafeUrl {
   ...
   */
  static revokeObjectUrl(safeUrl) {
   ...
   */
  static fromMediaSource(mediaSource) {
    goog_asserts.assert(
        'MediaSource' in window, 'No support for MediaSource');
  ...
  static tryFromDataUrl(dataUrl) {
    ...
    var valid = match && SafeUrl.isSafeMimeType(match[1]);
    if (valid) {
      return SafeUrl.createSafeUrlSecurityPrivateDoNotAccessOrElse(
          filteredDataUrl);
    }
