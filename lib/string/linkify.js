      ...
      function(part, before, original, email, protocol) {
        output.push(
            opt_preserveNewlines ?
                SafeHtml.htmlEscapePreservingNewlines(before) :
                before);
        if (!original) {
          return '';
        }
        ...
        if (email) {
          ...
          if (splitEndingPunctuation && !strings.contains(original, '(')) {
        ...
        output.push(
            opt_preserveNewlines ?
                SafeHtml.htmlEscapePreservingNewlines(afterLink) :
                afterLink);
        return '';
      });
...
function findFirstEmail(text) {
  var email = text.match(EMAIL_RE_);
