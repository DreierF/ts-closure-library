 ...
 * @private
 */
let MS_PER_GREGORIAN_CYCLE_ = 146097 * 24 * 60 * 60 * 1000;
...
function setIso8601TimeOnly_(d, formatted) {
  ...
  if (timezoneParts) {
    ...
    let utc = Date.UTC(year, monthNumber, day, hour, minute, second, millisecond);
    if (twoDigitYear) {
 ...
 * @return {string}
 */
function padYear_(year) {
  const sign = year < 0 ? '-' : year >= 10000 ? '+' : '';
  return sign + strings.padNumber(Math.abs(year), sign ? 6 : 4);
};
...
class date_Date {
  ...
  toIsoString(opt_verbose, opt_tz) {
    var str = [
      ...
      strings.padNumber(this.getMonth() + 1, 2),
  ...
  toUTCIsoString(opt_verbose, opt_tz) {
    var str = [
      ...
      strings.padNumber(this.getUTCMonth() + 1, 2),
...
export {DateLike, DateTime, Interval, MS_PER_DAY, date_Date as Date, getNumberOfDaysInMonth, getWeekNumber, getYearOfWeek, isLeapYear, isLongIsoYear, isSameDay, isSameMonth, isSameYear, max, min, month, setIso8601DateTime, weekDay};