...
import {DateTimeFormat as I18n_DateTimeFormat} from './datetimeformat.js';
import {Format} from './datetimeformat.js';
...
class DateTimeParse {
  ...
  applyStandardPattern_(formatType) {
    ...
    if (formatType > Format.SHORT_DATETIME) {
      formatType = Format.MEDIUM_DATETIME;
    }
  ...
  internalParse_(
    ...
    var cal = new DateTimeParse.MyDate_();
    var parsePos = [start];
   ...
   * @private
   */
  getNextCharCount_(pattern, start) {
   ...
   * @private
   */
  isNumericField_(part) {
    if (part.count <= 0) {
      return false;
    }
    var i = DateTimeParse.NUMERIC_FORMAT_CHARS_.indexOf(
        part.text.charAt(0));
 ...
 * @private
 */
DateTimeParse.PATTERN_CHARS_ = 'GyMdkHmsSEDahKzZvQL';
...
DateTimeParse.MyDate_ = class {
  ...
  calcDate_(
    ...
    if (this.day != undefined) {
      ...
      var maxDate =
          goog_date.getNumberOfDaysInMonth(date.getFullYear(), date.getMonth());
      date.setDate(orgDate > maxDate ? maxDate : orgDate);
    }
  
    if (google.isFunction(date.setHours)) {
    ...
    if (google.isFunction(date.setMinutes) && this.minutes != undefined) {
      date.setMinutes(this.minutes);
    }
  
    if (google.isFunction(date.setSeconds) && this.seconds != undefined) {
      date.setSeconds(this.seconds);
    }
  
    if (google.isFunction(date.setMilliseconds) && this.milliseconds != undefined) {
