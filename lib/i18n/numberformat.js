...
import * as i18ncurrency from './currency.js';
import {CurrencyInfo} from './currency.js';
import {NumberFormatSymbols as I18n_NumberFormatSymbols} from './numberformatsymbols.js';
import {NumberFormatSymbols_u_nu_latn} from './numberformatsymbols.js';
...
class NumberFormat {
  ...
  constructor(
    ...
    if (opt_currency && !i18ncurrency.isValid(opt_currency)) {
    ...
    this.currencyStyle_ =
        opt_currencyStyle || CurrencyStyle.LOCAL;
   ...
   *     enforced.
   */
  static setEnforceAsciiDigits(doEnforce) {
   ...
   * @private
   */
  setFormatterToPolyfill_(pattern) {
    // JavaScript implementation for older browswer or a string pattern.
    this.intlFormatter_ = null;
   ...
   */
  roundNumber_(number) {
    const shift = NumberFormat.decimalShift_;
   ...
   * @private
   */
  formatNumberGroupingRepeatingDigitsParts_(parts, zeroCode, intPart, groupingArray, repeatedDigitLen) {
   ...
   * @private
   */
  formatNumberGroupingNonRepeatingDigitsParts_(parts, zeroCode, intPart, groupingArray) {
    // Keep track of how much has been completed on the non repeated groups
    const grouping = this.getNumberFormatSymbols_().GROUP_SEP;
  ...
  subformatFixed_(
    ...
    if (fracPartSplit.length == 2) {
      ...
      fracPart += strings.repeat('0', exp - fracPart.length + 1);
    }
    ...
    if (this.maximumFractionDigits_ + 1 > fracPart.length) {
      ...
      fracPart = '1' + strings.repeat('0', zeroesToAdd) + fracPart;
    }
   ...
   * @private
   */
  addExponentPart_(exponent, parts) {
          ...
          case NumberFormat.PATTERN_CURRENCY_SIGN_:
            if ((pos[0] + 1) < len &&
              ...
              switch (this.currencyStyle_) {
                case CurrencyStyle.LOCAL:
                  affix += i18ncurrency.getLocalCurrencySignWithFallback(
                      this.getCurrencyCode_());
                  break;
                case CurrencyStyle.GLOBAL:
                  affix += i18ncurrency.getGlobalCurrencySignWithFallback(
                      this.getCurrencyCode_());
                  break;
                case CurrencyStyle.PORTABLE:
                  affix += i18ncurrency.getPortableCurrencySignWithFallback(
                      this.getCurrencyCode_());
                  break;
                default:
                  break;
   ...
   * @private
   */
  static decimalShift_(number, digitCount) {
    asserts.assert(
   ...
   * @suppress {checkTypes}
   */
  static decimalRound_(number, decimalCount) {
    asserts.assert(
 ...
 * @private
 */
NumberFormat.NativeLocaleDigits_ = {
  'ar': 'latn',
  'ar-EG': 'arab',
  'bn': 'beng',
  'fa': 'arabext',
...
NumberFormat.prototype.SetUpIntlFormatter_ = function(inputPattern) {
  ...
  if (NumberFormat.isEnforceAsciiDigits()) {
  ...
  switch (inputPattern) {
    ...
    case Format.CURRENCY:
      ...
      const precision = i18ncurrency.isAvailable(currencyCode) ?
          CurrencyInfo[currencyCode][0] % 16 :
          2;
  ...
  try {
    ...
    let locale;
    if (google.LOCALE) {
      locale = google.LOCALE.replace('_', '-');
    }
    if (!NumberFormat.enforceAsciiDigits_ &&
        locale in NumberFormat.NativeLocaleDigits_) {
      // Sets native digits for same locales as polyfill.
      options.numberingSystem =
          NumberFormat.NativeLocaleDigits_[locale];
    }
...
export {CurrencyStyle, Format, NumberFormat};