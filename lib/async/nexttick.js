...
function nextTick(callback, opt_context, opt_useSetImmediate) {
  ...
  // for details.
  if (google.isFunction(window.setImmediate) &&
  ...
  if (!nextTick.setImmediate_) {
    nextTick.setImmediate_ =
        nextTick.getSetImmediateEmulator_();
  }
  nextTick.setImmediate_(cb);
};
 ...
 * @private
 */
nextTick.getSetImmediateEmulator_ = function() {
  ...
  if (typeof Channel === 'undefined' && typeof window !== 'undefined' &&
    ...
    Channel = function() {
      ...
      safe.setIframeSrc(
          iframe,
          TrustedResourceUrl.fromConstant(Const.EMPTY));
      document.documentElement.appendChild(iframe);
      var win = iframe.contentWindow;
      var doc = win.document;
      doc.open();
      safe.documentWrite(doc, SafeHtml.EMPTY);
      doc.close();
  ...
  if (typeof document !== 'undefined' &&
      'onreadystatechange' in dom.createElement(TagName.SCRIPT)) {
    return function(cb) {
      var script = dom.createElement(TagName.SCRIPT);
...
entryPointRegistry.register(
     ...
     *     function.
     */
    function(transformer) { nextTick.wrapCallback_ = transformer; });

export {nextTick, throwException};