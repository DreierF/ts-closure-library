...
import {Role as A11yariaRole} from './../a11y/aria/roles.js';
...
function getDefaultRenderer(componentCtor) {
  ...
  while (ctor) {
    key = google.getUid(ctor);
    if ((rendererCtor = defaultRenderers_[key])) break;
    ctor = /** @type {?Function|undefined} */ (goog_object.getSuperClass(ctor));
  }
...
class Control extends Ui_Component {
  ...
  enableMouseEventHandling_(enable) {
    ...
    if (enable) {
      ...
      if (this.handleContextMenu != goog_functions.UNDEFINED) {
        handler.listen(
  ...
  getCaption() {
    ...
    var caption = (typeof content === 'string') ?
        ...
        content.map(goog_dom.getRawTextContent).join('') :
   ...
   */
  handleContextMenu(e) {
    return goog_functions.UNDEFINED(e);
  }
