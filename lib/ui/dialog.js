...
class Dialog extends ModalPopup {
  ...
  constructor(opt_class, opt_useIframeMask, opt_domHelper) {
     ...
     * @private
      * @type {boolean}
     */
    this.trackSurroundingSpace_ = false;
     ...
     * @private
     */
    this.isAriaDescribedByContent_ = false;
  ...
  setDraggingEnabled_(enabled) {
    ...
    if (enabled && !this.dragger_) {
      ...
      goog_events.listen(
          ...
          false, this);
      goog_events.listen(
          this.dragger_, DraggerEventType.DRAG, this.handleDrag, false,
          this);
  ...
  createDom() {
    ...
    googdom.append(
        ...
        this.contentEl_ = dom.createDom(TagName.DIV, {
          'className': google.getCssName(this.class_, 'content'),
          'id': contentElId
        }),
    ...
    if (e.type == EventsEventType.KEYDOWN) {
      ...
      if (this.escapeToCancel_ && e.keyCode == KeyCodes.ESC) {
        ...
        if (cancel && !isSpecialFormElement) {
          ...
          close = this.dispatchEvent(new Dialog_Event(
              cancel,
