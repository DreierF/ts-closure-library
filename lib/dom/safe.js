...
function setInnerHtml(elem, html) {
  if (googasserts.ENABLE_ASSERTS && /** @type {?} */ (elem).tagName) {
 ...
 */
function openInWindow(url, opt_openerWin, opt_name, opt_specs) {
  /** @type {!Html_SafeUrl} */
  var safeUrl;
  if (url instanceof Html_SafeUrl) {
    safeUrl = url;
  ...
  if (opt_specs !== undefined) {
    return win.open(Html_SafeUrl.unwrap(safeUrl), name, opt_specs);
  } else {
