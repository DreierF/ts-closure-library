...
function setInnerHtml(elem, html) {
  if (googasserts.ENABLE_ASSERTS && elem.tagName) {
 ...
 *     template.
 */
function setInnerHtmlFromConstant(element, constHtml) {
  setInnerHtml(
      element,
...
function setNonceForScriptElement_(script) {
  ...
  var nonce = google.getScriptNonce(win);
  if (nonce) {
...
function openInWindow(
  ...
  if (opt_specs !== undefined || opt_replace !== undefined) {
    return win.open(
        Html_SafeUrl.unwrap(safeUrl), name, opt_specs, opt_replace);
  } else {
    return win.open(Html_SafeUrl.unwrap(safeUrl), name);
  }
};
 ...
 * @param {!Html_SafeHtml} html HTML to create a fragment from.
 * @return {?DocumentFragment}
 */
function createContextualFragment(range, html) {
  return range.createContextualFragment(
      Html_SafeHtml.unwrapTrustedHTML(html));
};
 ...
 */
function getStyleNonce(opt_window) {
  if (opt_window && opt_window != window) {
    return getNonce_(opt_window.document, 'style');
  }
  if (cspStyleNonce_ === null) {
    cspStyleNonce_ =
        getNonce_(window.document, 'style');
  }
  return cspStyleNonce_;
};

/** @private {?string} */
let cspStyleNonce_ = null;
...
export {InsertAdjacentHtmlPosition, assignLocation, createContextualFragment, createImageFromBlob, documentWrite, getStyleNonce, insertAdjacentHtml, openInWindow, parseFromString, parseFromStringHtml, replaceLocation, setAnchorHref, setAudioSrc, setButtonFormAction, setEmbedSrc, setFormElementAction, setFrameSrc, setIframeSrc, setIframeSrcdoc, setImageSrc, setInnerHtml, setInnerHtmlFromConstant, setInputFormAction, setLinkHrefAndRel, setLocationHref, setObjectData, setOuterHtml, setScriptContent, setScriptSrc, setStyle, setVideoSrc, unsafeSetInnerHtmlDoNotUseOrElse};