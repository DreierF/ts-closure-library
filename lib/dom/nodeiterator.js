 ...
 * @final
 */
class NodeIterator extends TagIterator {
  ...
  constructor(
    ...
    super(opt_node, opt_reversed, opt_unconstrained, null, opt_depth);
  }
  ...
  next() {
    do {
      try {
        super.nextValueOrThrow();
      } catch (ex) {
        if (ex === StopIteration) return goog_iter.ES6_ITERATOR_DONE;
        throw ex;
      }
    } while (this.isEndTag());
  
    return goog_iter.createEs6IteratorYield(/** @type {!Node} */ (this.node));
  };

  /**
   * TODO(user): Please do not remove - this will be cleaned up centrally.
   * @override @see {!IterIterator}
   * @return {!Node}
   */
  nextValueOrThrow() {
    return goog_iter.toEs4IteratorNext(
        NodeIterator.prototype.next.call(this));
  };
}
