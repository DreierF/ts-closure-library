...
function add_(classes, args) {
  for (var i = 0; i < args.length; i++) {
    if (!googarray.contains(classes, args[i])) {
      classes.push(args[i]);
    }
  }
};
...
function getDifference_(arr1, arr2) {
  return googarray.filter(
      arr1, function(item) { return !googarray.contains(arr2, item); });
};
...
function addRemove(element, classesToRemove, classesToAdd) {
  ...
  if (typeof classesToRemove === 'string') {
    googarray.remove(classes, classesToRemove);
  } else if (google.isArray(classesToRemove)) {
    classes = getDifference_(classes, classesToRemove);
  }

  if (typeof classesToAdd === 'string' &&
      !googarray.contains(classes, classesToAdd)) {
    classes.push(classesToAdd);
  } else if (google.isArray(classesToAdd)) {
...
export {add, addRemove, enable, get, has, remove, set, swap, toggle};