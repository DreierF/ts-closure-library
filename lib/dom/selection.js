 ...
 * @private
 */
function getEndPointsTextareaIe_(
 ...
 */
function getEndPoints(textfield) {
  ...
  if (useSelectionProperties_(textfield)) {
    ...
    if (range.inRange(selectionRange)) {
      ...
      if (textfield.type == InputType.TEXTAREA) {
        return getEndPointsTextareaIe_(
...
function setEnd(textfield, pos) {
  if (useSelectionProperties_(textfield)) {
    ...
    if (range.inRange(selectionRange)) {
      ...
      pos = canonicalizePositionIe_(textfield, pos);
      var startCursorPos = canonicalizePositionIe_(
          textfield, getStart(textfield));
 ...
 */
function setCursorPosition(textfield, pos) {
  if (useSelectionProperties_(textfield)) {
...
function getText(textfield) {
  ...
  if (isLegacyIe_()) {
    ...
    if (!range.inRange(selectionRange)) {
      return '';
    } else if (textfield.type == InputType.TEXTAREA) {
      return getSelectionRangeText_(selectionRange);
    }
...
function canonicalizePositionIe_(textfield, pos) {
  ...
  if (textfield.type == InputType.TEXTAREA) {
    ...
    pos = strings.canonicalizeNewlines(value).length;
  }
  return pos;
};
 ...
 * @private
 */
function isLegacyIe_() {
  return userAgent.IE && !userAgent.isVersionOrHigher('9');
};

export {getEnd, getEndPoints, getStart, getText, setCursorPosition, setEnd, setStart, setText};