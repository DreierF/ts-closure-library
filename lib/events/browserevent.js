 ...
 * as usual.
 */
const USE_LAYER_XY_AS_OFFSET_XY = false;
 ...
 * @extends {EventsEvent}
 */
class events_BrowserEvent extends EventsEvent {
  ...
  init(e, opt_currentTarget) {
    ...
    this.pointerType = events_BrowserEvent.getPointerType_(e);
    this.state = e.state;
    this.event_ = e;
  ...
  isButton(button) {
    if (!BrowserFeature.HAS_W3C_BUTTON) {
      if (this.type == 'click') {
        ...
        return !!(
            this.event_.button & events_BrowserEvent.IE_BUTTON_MAP[button]);
      }
    } else {
  ...
  isMouseActionButton() {
    ...
    return this.isButton(MouseButton.LEFT) &&
        !(userAgent.WEBKIT && userAgent.MAC && this.ctrlKey);
  };
 ...
 * @type {!Array<number>}
 * @deprecated Use `events_BrowserEvent.IE_BUTTON_MAP` instead.
 */
events_BrowserEvent.IEButtonMap = debug.freeze([
  1,  // LEFT
  4,  // MIDDLE
  2   // RIGHT
]);
 ...
 * @const {!Array<number>}
 */
events_BrowserEvent.IE_BUTTON_MAP = events_BrowserEvent.IEButtonMap;

/**
 * Static data for mapping MSPointerEvent types to PointerEvent types.
 * @const {!Object<number, PointerType>}
 */
events_BrowserEvent.IE_POINTER_TYPE_MAP = debug.freeze({
  2: PointerType.TOUCH,
  3: PointerType.PEN,
  4: PointerType.MOUSE
});
 ...
 * @private
 */
events_BrowserEvent.getPointerType_ = function(e) {
  if (typeof (e.pointerType) === 'string') {
    return e.pointerType;
  }
  // IE10 uses integer codes for pointer type.
  // https://msdn.microsoft.com/en-us/library/hh772359(v=vs.85).aspx
  return events_BrowserEvent.IE_POINTER_TYPE_MAP[e.pointerType] || '';
};

export {MouseButton, PointerType, USE_LAYER_XY_AS_OFFSET_XY, events_BrowserEvent as BrowserEvent};