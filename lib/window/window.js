 ...
 * @type {boolean}
 */
const USE_EMPTY_STRING_FOR_BLANK_URL = true;
...
function open(linkRef, opt_options, opt_parentWin) {
  ...
  const browserSupportsCoop =
      ENFORCE_COOP && self.crossOriginIsolated !== undefined;
...
function openBlank(opt_message, opt_options, opt_parentWin) {
  ...
  let url = '';
  if (opt_message || !USE_EMPTY_STRING_FOR_BLANK_URL) {
...
export {DEFAULT_POPUP_HEIGHT, DEFAULT_POPUP_TARGET, DEFAULT_POPUP_WIDTH, ENFORCE_COOP, USE_EMPTY_STRING_FOR_BLANK_URL, open, openBlank, popup};