...
class goog_Promise {
  ...
  constructor(resolver, opt_context) {
    ...
    // goog_functions.UNDEFINED. This value is passed internally when creating a
    // promise which will be resolved through a more optimized path.
    if (resolver != goog_functions.UNDEFINED) {
      try {
        var self = this;
        resolver.call(
            opt_context,
  ...
  static resolve(opt_value) {
    ...
    // Passing goog_functions.UNDEFINED will cause the constructor to take an
    // optimized path that skips calling the resolver function.
    var promise = new goog_Promise(goog_functions.UNDEFINED);
  ...
  thenVoid(
    ...
    this.addCallbackEntry_(goog_Promise.getCallbackEntry_(
        opt_onFulfilled || (goog_functions.UNDEFINED), opt_onRejected || null,
        opt_context));
  };
   ...
   * @template THIS
   */
  catch(onRejected, opt_context) {
    return this.thenCatch(onRejected, opt_context);
  }
  ...
  addCallbackEntry_(callbackEntry) {
    if (!this.hasEntry_() &&
        (this.state_ == goog_Promise.State_.FULFILLED ||
...
goog_Promise.freelist_ = new FreeList(
    function() {
      return new goog_Promise.CallbackEntry_();
    },
    function(item) {
      item.reset();
    },
