...
import {Thenable as Goog_Thenable} from './thenable.js';
/**
 * @license
...
class goog_Promise {
   ...
   * @private
   */
  static getCallbackEntry_(onFulfilled, onRejected, context) {
   ...
   * thenable.
   *
   * @param {?(Goog_Thenable<TYPE>|Thenable|TYPE)} value
   * @param {function(TYPE): ?} onFulfilled
   * @param {function(*): *} onRejected
   * @template TYPE
   * @private
   */
  static resolveThen_(value, onFulfilled, onRejected) {
    var isThenable =
  ...
  /**
   * @param {!Array<?(goog_Promise<TYPE>|Goog_Thenable<TYPE>|Thenable|*)>}
   *     promises
   ...
   * @template TYPE
   */
  static firstFulfilled(promises) {
    return new goog_Promise(function(resolve, reject) {
   ...
   * @private
   */
  static maybeThen_(value, onFulfilled, onRejected, context) {
    if (value instanceof goog_Promise) {
      value.thenVoid(onFulfilled, onRejected, context);
      return true;
    } else if (Goog_Thenable.isImplementedBy(value)) {
      value = /** @type {!Goog_Thenable} */ (value);
   ...
   */
  static setUnhandledRejectionHandler(handler) {
...
Goog_Thenable.addImplementation(goog_Promise);
...
export {DEFAULT_MAX_UNUSED, LONG_STACK_TRACES, UNHANDLED_REJECTION_DELAY, goog_Promise as Promise};