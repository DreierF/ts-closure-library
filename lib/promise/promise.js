...
class goog_Promise {
  ...
  constructor(resolver, opt_context) {
     ...
     */
    this.state_ = goog_Promise.State_.PENDING;
    ...
    if (UNHANDLED_REJECTION_DELAY > 0) {
      /**
    ...
    } else if (UNHANDLED_REJECTION_DELAY == 0) {
      /**
    ...
    if (resolver != google.nullFunction) {
      try {
        var self = this;
        resolver.call(
            opt_context,
            function(value) {
              self.resolve_(goog_Promise.State_.FULFILLED, value);
            },
            function(reason) {
              if (google.DEBUG &&
                  !(reason instanceof goog_Promise.CancellationError)) {
                try {
              ...
              self.resolve_(goog_Promise.State_.REJECTED, reason);
            });
      } catch (e) {
        this.resolve_(goog_Promise.State_.REJECTED, e);
      }
    }
  }
  ...
  then(
    ...
    return this.addChildPromise_(
        google.isFunction(opt_onFulfilled) ? opt_onFulfilled : null,
        google.isFunction(opt_onRejected) ? opt_onRejected : null, opt_context);
  };
  ...
  thenVoid(
    ...
    this.addCallbackEntry_(
        goog_Promise.getCallbackEntry_(
            opt_onFulfilled || google.nullFunction, opt_onRejected || null,
            opt_context));
  };
   ...
   * @template THIS
   */
  thenAlways(onSettled, opt_context) {
    if (LONG_STACK_TRACES) {
      this.addStackTrace_(new Error('thenAlways'));
    }
  
    var entry = goog_Promise.getCallbackEntry_(onSettled, onSettled, opt_context);
    entry.always = true;
   ...
   * @template THIS
   */
  thenCatch(onRejected, opt_context) {
    if (LONG_STACK_TRACES) {
   ...
   */
  cancel(opt_message) {
    if (this.state_ == goog_Promise.State_.PENDING) {
      // Instantiate Error object synchronously. This ensures Error::stack points
      // to the cancel() callsite.
      var err = new goog_Promise.CancellationError(opt_message);
      run(function() {
  ...
  cancelInternal_(err) {
    if (this.state_ == goog_Promise.State_.PENDING) {
      if (this.parent_) {
        ...
        this.resolve_(goog_Promise.State_.REJECTED, err);
      }
    }
  };
  ...
  cancelChild_(childPromise, err) {
    ...
    if (childEntry) {
      if (this.state_ == goog_Promise.State_.PENDING && childCount == 1) {
        this.cancelInternal_(err);
      } else {
        if (beforeChildEntry) {
        ...
        this.executeCallback_(childEntry, goog_Promise.State_.REJECTED, err);
      }
    }
  };
  ...
  addCallbackEntry_(callbackEntry) {
    if (!this.hasEntry_() && (this.state_ == goog_Promise.State_.FULFILLED ||
                              this.state_ == goog_Promise.State_.REJECTED)) {
  ...
  addChildPromise_(
    ...
    var callbackEntry = goog_Promise.getCallbackEntry_(null, null, null);
  
    callbackEntry.child = new goog_Promise(function(resolve, reject) {
  ...
  unblockAndFulfill_(value) {
    asserts.assert(this.state_ == goog_Promise.State_.BLOCKED);
    this.state_ = goog_Promise.State_.PENDING;
    this.resolve_(goog_Promise.State_.FULFILLED, value);
  };
  ...
  unblockAndReject_(reason) {
    asserts.assert(this.state_ == goog_Promise.State_.BLOCKED);
    this.state_ = goog_Promise.State_.PENDING;
    this.resolve_(goog_Promise.State_.REJECTED, reason);
  };
  ...
  resolve_(state, x) {
    ...
    if (this === x) {
      state = goog_Promise.State_.REJECTED;
      x = new TypeError('Promise cannot resolve to itself');
    }
  
    this.state_ = goog_Promise.State_.BLOCKED;
    var isThenable = goog_Promise.maybeThen_(
    ...
    if (state == goog_Promise.State_.REJECTED &&
        !(x instanceof goog_Promise.CancellationError)) {
      goog_Promise.addUnhandledRejection_(this, x);
    }
  };
  ...
  executeCallback_(
    ...
    if (state == goog_Promise.State_.REJECTED && callbackEntry.onRejected &&
    ...
    if (callbackEntry.child) {
      ...
      goog_Promise.invokeCallback_(callbackEntry, state, result);
    } else {
      ...
      try {
        ...
        goog_Promise.handleRejection_.call(null, err);
      }
    }
    goog_Promise.returnEntry_(callbackEntry);
  };
   ...
   */
  addStackTrace_(err) {
    if (LONG_STACK_TRACES && typeof err.stack === 'string') {
  ...
  appendLongStack_(err) {
    if (LONG_STACK_TRACES && err && typeof err.stack === 'string' &&
        this.stack_.length) {
      ...
      for (var promise = this; promise; promise = promise.parent_) {
        ...
        longTrace.push(
            'Value: ' +
            '[' + (promise.state_ == goog_Promise.State_.REJECTED ? 'REJECTED' :
                                                                    'FULFILLED') +
            '] ' +
  ...
  removeUnhandledRejection_() {
    if (UNHANDLED_REJECTION_DELAY > 0) {
      for (var p = this; p && p.unhandledRejectionId_; p = p.parent_) {
        window.clearTimeout(p.unhandledRejectionId_);
 ...
 */
const UNHANDLED_REJECTION_DELAY = 0;
 ...
 *
 * @private @final @class */
goog_Promise.CallbackEntry_ = class {
...
/** @const @private {FreeList<!goog_Promise.CallbackEntry_>} */
goog_Promise.freelist_ = new FreeList(
    function() { return new goog_Promise.CallbackEntry_(); },
 ...
 * @private
 */
goog_Promise.getCallbackEntry_ = function(onFulfilled, onRejected, context) {
  var entry = goog_Promise.freelist_.get();
 ...
 * @private
 */
goog_Promise.returnEntry_ = function(entry) {
  goog_Promise.freelist_.put(entry);
};
 ...
 * =:
 */
goog_Promise.resolve = function(opt_value) {
  if (opt_value instanceof goog_Promise) {
    // Avoid creating a new object if we already have a promise object
    // of the correct type.
    return opt_value;
  }

  // Passing google.nullFunction will cause the constructor to take an optimized
  // path that skips calling the resolver function.
  var promise = new goog_Promise(google.nullFunction);
  promise.resolve_(goog_Promise.State_.FULFILLED, opt_value);
  return promise;
};
 ...
 *     given reason.
 */
goog_Promise.reject = function(opt_reason) {
  return new goog_Promise(function(resolve, reject) { reject(opt_reason); });
};
 ...
 * @private
 */
goog_Promise.resolveThen_ = function(value, onFulfilled, onRejected) {
  var isThenable =
      goog_Promise.maybeThen_(value, onFulfilled, onRejected, null);
  if (!isThenable) {
    run(google.partial(onFulfilled, value));
  }
};
 ...
 * @template TYPE
 */
goog_Promise.race = function(promises) {
  return new goog_Promise(function(resolve, reject) {
    if (!promises.length) {
      resolve(undefined);
    }
    for (var i = 0, promise; i < promises.length; i++) {
      promise = promises[i];
      goog_Promise.resolveThen_(promise, resolve, reject);
    }
  });
};
 ...
 * @template TYPE
 */
goog_Promise.all = function(promises) {
  return new goog_Promise(function(resolve, reject) {
    ...
    function onFulfill(index, value) {
      toFulfill--;
      values[index] = value;
      if (toFulfill == 0) {
        resolve(values);
    ...
    function onReject(reason) { reject(reason); };

    for (var i = 0, promise; i < promises.length; i++) {
      promise = promises[i];
      goog_Promise.resolveThen_(promise, google.partial(onFulfill, i), onReject);
    }
  });
};
 ...
 * @template TYPE
 */
goog_Promise.allSettled = function(promises) {
  return new goog_Promise(function(resolve, reject) {
    ...
    function onSettled(index, fulfilled, result) {
      toSettle--;
    ...
    for (var i = 0, promise; i < promises.length; i++) {
      promise = promises[i];
      goog_Promise.resolveThen_(
          promise, google.partial(onSettled, i, true /* fulfilled */),
          google.partial(onSettled, i, false /* fulfilled */));
    }
  });
};
 ...
 * @template TYPE
 */
goog_Promise.firstFulfilled = function(promises) {
  return new goog_Promise(function(resolve, reject) {
    ...
    function onFulfill(value) { resolve(value); };

    function onReject(index, reason) {
      toReject--;
    ...
    for (var i = 0, promise; i < promises.length; i++) {
      promise = promises[i];
      goog_Promise.resolveThen_(promise, onFulfill, google.partial(onReject, i));
    }
  });
};
 ...
 * @template TYPE
 */
goog_Promise.withResolver = function() {
  var resolve, reject;
  var promise = new goog_Promise(function(rs, rj) {
    resolve = rs;
    reject = rj;
  });
  return new goog_Promise.Resolver_(promise, resolve, reject);
};

Thenable.addImplementation(goog_Promise);
 ...
 * @private
 */
goog_Promise.maybeThen_ = function(value, onFulfilled, onRejected, context) {
  if (value instanceof goog_Promise) {
    ...
    return true;
  } else if (google.isObject(value)) {
    const thenable = /** @type {!Thenable} */ (value);
    try {
      var then = thenable.then;
      if (google.isFunction(then)) {
        goog_Promise.tryThen_(thenable, then, onFulfilled, onRejected, context);
        return true;
      }
    } catch (e) {
 ...
 * @private
 */
goog_Promise.tryThen_ = function(
 ...
 * @private
 */
goog_Promise.invokeCallback_ = function(callbackEntry, state, result) {
  if (state == goog_Promise.State_.FULFILLED) {
 ...
 * @private
 */
goog_Promise.addUnhandledRejection_ = function(promise, reason) {
  if (UNHANDLED_REJECTION_DELAY > 0) {
    promise.unhandledRejectionId_ = window.setTimeout(function() {
      promise.appendLongStack_(reason);
      goog_Promise.handleRejection_.call(null, reason);
 ...
 * @private
 */
goog_Promise.handleRejection_ = throwException;
 ...
 */
goog_Promise.setUnhandledRejectionHandler = function(handler) {
  goog_Promise.handleRejection_ = handler;
};
 ...
 * @final
 */
goog_Promise.CancellationError = class extends DebugError {
...
export {DEFAULT_MAX_UNUSED, LONG_STACK_TRACES, Resolver, Thenable, UNHANDLED_REJECTION_DELAY, goog_Promise as Promise};