 ...
 */
const NATIVE_ARRAY_PROTOTYPES = google.TRUSTED_SITE;
 ...
 */
const ASSUME_NATIVE_FUNCTIONS = google.FEATURESET_YEAR > 2012;
 ...
 * @template T
 */
let indexOf = NATIVE_ARRAY_PROTOTYPES &&
 ...
 * @template T
 */
let lastIndexOf = NATIVE_ARRAY_PROTOTYPES &&
 ...
 * @template T,S
 */
let forEach = NATIVE_ARRAY_PROTOTYPES &&
 ...
 * @template T,S
 */
let filter = NATIVE_ARRAY_PROTOTYPES &&
 ...
 */
let map = NATIVE_ARRAY_PROTOTYPES &&
 ...
 * @template T,S,R
 */
let reduce = NATIVE_ARRAY_PROTOTYPES &&
 ...
 * @template T,S,R
 */
let reduceRight = NATIVE_ARRAY_PROTOTYPES &&
 ...
 * @template T,S
 */
let some = NATIVE_ARRAY_PROTOTYPES &&
 ...
 * @template T,S
 */
let every = NATIVE_ARRAY_PROTOTYPES &&
...
function binarySearch(arr, target, opt_compareFn) {
  return binarySearch_(
      arr, opt_compareFn || defaultCompare, false /* isEvaluator */,
      target);
};
...
function sortObjectsByKey(arr, key, opt_compareFn) {
  sortByKey(arr, function(obj) { return obj[key]; }, opt_compareFn);
};
...
function moveItem(arr, fromIndex, toIndex) {
  ...
  // We don't use insertAt and removeAt, because they're
...
function copyByIndex(arr, index_arr) {
  var result = [];
  forEach(index_arr, function(index) { result.push(arr[index]); });
  return result;
};
...
export {ASSUME_NATIVE_FUNCTIONS, NATIVE_ARRAY_PROTOTYPES, binaryInsert, binaryRemove, binarySearch, binarySelect, bucket, clear, clone, compare3, concat, concatMap, contains, copyByIndex, count, defaultCompare, defaultCompareEquality, equals, every, extend, filter, find, findIndex, findIndexRight, findRight, flatten, forEach, forEachRight, indexOf, insert, insertArrayAt, insertAt, insertBefore, inverseDefaultCompare, isEmpty, isSorted, join, last, lastIndexOf, map, moveItem, peek, range, reduce, reduceRight, remove, removeAllIf, removeAt, removeDuplicates, removeIf, removeLast, repeat, rotate, shuffle, slice, some, sort, sortByKey, sortObjectsByKey, splice, stableSort, toArray, toObject, zip};