...
function isEmpty(col) {
  ...
  if (google.isArrayLike(col) || typeof col === 'string') {
    return googarray.isEmpty(/** @type {!Array<?>} */ (col));
  }
...
function forEach(col, f, opt_obj) {
  if (col.forEach && typeof col.forEach == 'function') {
    ...
    googarray.forEach(/** @type {!Array<?>} */ (col), f, opt_obj);
  } else {
...
function filter(col, f, opt_obj) {
  ...
  if (google.isArrayLike(col) || typeof col === 'string') {
    return googarray.filter(/** @type {!Array<?>} */ (col), f, opt_obj);
  }
  ...
  if (keys) {
    ...
    // We should not use googarray.filter here since we want to make sure that
...
function map(col, f, opt_obj) {
  ...
  if (google.isArrayLike(col) || typeof col === 'string') {
    return googarray.map(/** @type {!Array<?>} */ (col), f, opt_obj);
  }
  ...
  if (keys) {
    ...
    // We should not use googarray.map here since we want to make sure that
    // the index is undefined as well as make sure that col is passed to the
    // function.
    rv = [];
    for (var i = 0; i < l; i++) {
...
function some(col, f, opt_obj) {
  ...
  if (google.isArrayLike(col) || typeof col === 'string') {
    return googarray.some(/** @type {!Array<?>} */ (col), f, opt_obj);
  }
  var keys = getKeys(col);
  var values = getValues(col);
  var l = values.length;
...
function every(col, f, opt_obj) {
  ...
  if (google.isArrayLike(col) || typeof col === 'string') {
    return googarray.every(/** @type {!Array<?>} */ (col), f, opt_obj);
  }
  var keys = getKeys(col);
  var values = getValues(col);
  var l = values.length;
