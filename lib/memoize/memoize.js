...
function memoize(f, opt_serializer) {
  const serializer = opt_serializer || memoize.simpleSerializer;
  ...
  return (/**
          ...
          function() {
            if (ENABLE_MEMOIZE) {
              ...
              // compatibility.
              const thisOrGlobal = this || window;
              // Maps the serialized list of args to the corresponding return
              // value.
              const cache = thisOrGlobal[memoize.CACHE_PROPERTY_] ||
                  (thisOrGlobal[memoize.CACHE_PROPERTY_] = {});
              const key = serializer(google.getUid(f), arguments);
 ...
 */
memoize.clearCache = function(cacheOwner) {
  cacheOwner[memoize.CACHE_PROPERTY_] = {};
};

/**
 * Name of the property used by memoize as cache.
 * @type {string}
 * @private
 */
memoize.CACHE_PROPERTY_ = 'closure_memoize_cache_';

/**
 * Simple and fast argument serializer function for memoize.
 ...
 */
memoize.simpleSerializer = function(functionUid, args) {
