...
function catchErrors(logFunc, opt_cancel, opt_target) {
  ...
  if (userAgent.WEBKIT && !userAgent.isVersionOrHigher('535.3')) {
    retVal = !retVal;
  }
...
function expose(obj, opt_showFn) {
  ...
  for (var x in obj) {
    if (!opt_showFn && google.isFunction(obj[x])) {
      continue;
    }
    var s = x + ' = ';
...
export {FORCE_SLOPPY_STACKS, LOGGING_ENABLED, MAX_STACK_DEPTH, catchErrors, deepExpose, enhanceError, enhanceErrorWithContext, expose, exposeArray, freeze, getFunctionName, getStacktrace, getStacktraceSimple, makeWhitespaceVisible, normalizeErrorObject, runtimeType};