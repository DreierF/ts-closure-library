...
class Trace_ {
  ...
  constructor() {
     ...
     */
    this.logger_ = goog_log.getLogger('goog.debug.Trace');
     ...
     */
    this.outstandingEvents_ = new StructsMap();
  ...
  startTracer(comment, opt_type) {
    ...
    DebugLogger.logToProfilers('Start : ' + comment);
  
    /** @const */
    var event =
        /** @type {!Event_} */ (this.eventPool_.getObject());
  ...
  stopTracer(id, opt_silenceThreshold) {
    // this used to call google.isDef(opt_silenceThreshold) but that causes an
    ...
    if (stopEvent) {
      DebugLogger.logToProfilers('Stop : ' + stopEvent.comment);
    ...
    if (opt_timeStamp) {
      ...
      for (var i = 0; i < numEvents; i++) {
        ...
        if (eventTime > timeStamp) {
          googarray.insertAt(this.events_, eventComment, i);
          break;
        }
      }
      if (i == numEvents) {
  ...
  getStat_(type) {
    ...
    if (!stat) {
      stat = /** @type {Event_} */ (this.statPool_.getObject());
      stat.type = type;
   ...
   * @private
   */
  static longToPaddedString_(v) {
    v = Math.round(v);
    // todo (pupius) - there should be a generic string in google.string for this
    var space = '';
    if (v < 1000) space = ' ';
    if (v < 100) space = '  ';
    if (v < 10) space = '   ';
