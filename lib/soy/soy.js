...
function renderElement(
  ...
  const html = ensureTemplateOutputHtml_(template(
      opt_templateData || defaultTemplateData_, opt_injectedData));
  safe.unsafeSetInnerHtmlDoNotUseOrElse(
...
function renderAsFragment(
  ...
  const dom = opt_domHelper || googdom.getDomHelper();
  const output = template(
      opt_templateData || defaultTemplateData_, opt_injectedData);
  const html = ensureTemplateOutputHtml_(output);
...
function convertToElement_(templateResult, opt_domHelper) {
  ...
  const html = ensureTemplateOutputHtml_(templateResult);
  assertFirstTagValid_(html.getTypedStringValue());
...
function assertFirstTagValid_(html) {
  if (asserts.ENABLE_ASSERTS) {
    const matches = html.match(INVALID_TAG_TO_RENDER_);
    asserts.assert(
...
export {CompatibleIj, IjData, StrictHtmlTemplate, StrictTemplate, TextTemplate, convertToElement, renderAsElement, renderAsFragment, renderElement, renderHtml};