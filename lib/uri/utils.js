 ...
 * @private
 */
let urlPackageSupportLoggingHandler_ = null;
 ...
 *     detected.
 */
function setUrlPackageSupportLoggingHandler(handler) {
...
function getParamValues(uri, keyEncoded) {
  ...
  while ((foundIndex = findParam_(
    ...
    foundIndex += keyEncoded.length + 1;
    result.push(
        goog_strings.urlDecode(uri.slice(foundIndex, Math.max(position, 0))));
  }
