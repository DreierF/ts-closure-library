 ...
 *
 * This package is deprecated in favour of the Closure URL package (google.url)
 ...
 * `uri_utils.setUrlPackageSupportLoggingHandler` for one way to identify
...
class Uri {
  ...
  constructor(opt_uri, opt_ignoreCase) {
    ...
    if (opt_uri instanceof Uri) {
      ...
      this.ignoreCase_ = !!opt_ignoreCase;
      this.queryData_ = new QueryData(null, this.ignoreCase_);
    }
  }
  ...
  setQueryData(queryData, opt_decode) {
    ...
    if (queryData instanceof QueryData) {
      ...
      this.queryData_ = new QueryData(queryData, this.ignoreCase_);
    }
...
class QueryData {
   ...
   *     name in #get.
   */
  constructor(opt_query, opt_ignoreCase) {
    /**
   ...
   */
  static createFromMap(map, opt_ignoreCase) {
    var keys = structs.getKeys(map);
    if (typeof keys == 'undefined') {
      throw new Error('Keys are undefined');
    }
  
    var queryData = new QueryData(null, opt_ignoreCase);
  ...
  toString() {
    ...
    for (var i = 0; i < keys.length; i++) {
      const key = keys[i];
      const encodedKey = goog_strings.urlEncode(key);
